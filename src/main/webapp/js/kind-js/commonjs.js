...
$(document).ready(function() {
	...
	if(global_page != "home") {
		...
		$(".prevskin").click(function() {
			...
			if(!$("#con_skins_1").is(":hidden")) {
				$("#skin_panel_1").animate({
					scrollLeft: "-=160"
				}, 200);
			} else {
				$("#skin_panel_2").animate({
					scrollLeft: "-=160"
				}, 200);
			}
		});
	...
	$(".login a").mousemove(function() {
		...
		$(this).addClass("current");
		$(this).parent().find(".downmenu1").slideDown("fast", function() {
...
function loadDoc() {
	...
	$.ajax({
		...
		success: function(msg) {
			...
			if(!IsPC()) {
				doc_Scroll = new IScroll('#wrapper_' + pc_id, {
			...
			$('.hprev').click(function() {
				left($(this).parent().find("div"));
			});
			$('.hnext').click(function() {
				right($(this).parent().find("div"));
			});
